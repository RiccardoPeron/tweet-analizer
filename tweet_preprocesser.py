...
class TweetPreprocessing:
    ...
    def __init__(self, file, nlp_=spacy.load("it_core_news_lg", exclude=['tagger', 'parser', 'lemmatizer', 'textcat', 'custom']), start_='0001-01-01', end_='9999-12-31'):
    ...
    def get_tweet_datas(self):
        ...
        for data in self.dataset:
            for tweet in data['data']:
                if (datetime.fromisoformat(tweet['created_at']).replace(tzinfo=None) >= self.start and
                        datetime.fromisoformat(tweet['created_at']).replace(tzinfo=None) <= self.end):
    ...
    def find_normalized(self, text, norm_text):
        ...
        money = []
        percent = []
        if ('<hashtag>' in norm_text or '<money>' in norm_text or '<percent>' in norm_text):
            for w in text:
                text = text.replace('+', '+ ')
                text = text.replace('-', '- ')
                text = text.replace('\'', '\' ')
    ...
    def generate_tweets_datas(self):
        ...
        '''
        tweets = []
        texts, ids, dates, context_annotaions, entities_annotations = self.get_tweet_datas()
        for i in range(len(texts)):
            tweets.append(self.generate_object(
                texts[i], ids[i], dates[i], context_annotaions[i], entities_annotations[i]))
        # print(tweets)
        return tweets
