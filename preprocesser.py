...
class TweeetPreprocesser:
    ...
    def extract_tag(self, text, normalized_text):
        if (
    ...
    def generate_object(
        ...
        ntext = self.normalizer(text)
        p, h, m, nt = self.extract_tag(text, ntext)
        nt = self.remove_tag(nt)
        tokens, labels, nv, lemma = self.get_tokens_arg(nt)
        nt.lower()
    ...
    def wrap_tweet(self, outname, folder):
        tweet_total = []
        sources = set()
...
def preprocess(folder_name, lan):
    ...
    for dir in os.listdir(path):
        ...
        for file in os.listdir(fpath):
            ...
            tweets = TP.generate_tweets_datas()
            TP.generate_json(
                tweets, "JSON_" + folder_name + "/" + str(year) + "/" + dir + "_sumup"
            )
    print("merging preprocessed tweets...")
    TP.wrap_tweet("merged_tweet_" + folder_name, "JSON_" + folder_name)
