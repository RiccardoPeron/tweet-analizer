...
class TweetAnalyzer:
    ...
    def __init__(self, file, first='', last=''):
        ...
        if first == '':
            self.__start = datetime.fromisoformat(
                min([tweet['created_at'] for tweet in self.data_full])).replace(tzinfo=None)
        else:
            self.__start = datetime.fromisoformat(first).replace(tzinfo=None)
        if last == '':
            self.__end = datetime.fromisoformat(
                max([tweet['created_at'] for tweet in self.data_full])).replace(tzinfo=None)
        else:
    ...
    def get_spacy_entities_labels(self):
        ...
        print('\t numero totale di entity labels non ripetute nello stesso tweet: {}'.format(
            sum(list(labels.values()))))
        print(
            '\t numero di tweet che hanno almeno una entity label: {} - {:.4}%'.format(tweets, tweets/self.total_tweets*100))
    ...
    def get_spacy_labels_stats(self, label):
        ...
        print('\t numero totale di entity labels con label {}: {} - {: .4} %'.format(
            label, all, all/self.total_tokens*100))

        print('\t numero totale di entity labels con label {} non ripetute nello stesso tweet: {}'.format(
            label, sum(list(categories_stats.values()))))
        print(
            '\t numero di tweet che hanno almeno una entity label con label {}: {} - {: .4} %'.format(label, tweets, tweets/self.total_tweets*100))
    ...
    def get_context_annotations_labels(self):
        ...
        print('\t numero totale di context annotations: {} - {:.4}%'.format(all,
              all/self.total_tokens*100))
        print('\t numero totale di context annotations non ripetute nello stesso tweet: {}'.format(
            sum(list(labels.values()))))
        print(
            '\t numero di tweet che hanno almeno una context annotation: {} - {:.4}%'.format(tweets, tweets/self.total_tweets*100))
        self.total_tweets = tweets
        self.total_tokens = all
        return self.sort_dict(labels)
    ...
    def get_context_label_stats(self, label):
        ...
        print('\t numero totale di context annotations con label {}: {} - {: .4} %'.format(
            label, all, all/self.total_tokens*100))
        print('\t numero totale di context annotations con label {} non ripetute nello stesso tweet: {}'.format(
            label, sum(list(categories_stats.values()))))
        print(
    ...
    def sort_dict(self, myDict, reverse_=True):
        ...
        '''
        return {k: v for k, v in sorted(myDict.items(), key=lambda item: item[1], reverse=reverse_)}
    ...
    def field_compatter(self, field, start, end):
        ...
        list_ = []
        for tweet in self.data_full:
            if (datetime.fromisoformat(tweet['created_at']).replace(tzinfo=None) >= start and
                    datetime.fromisoformat(tweet['created_at']).replace(tzinfo=None) <= end):
    ...
    def get_token_dict(self):
        ...
        ----------
        '''
        return self.sort_dict(Counter(self.unpack_list(self.field_compatter('tokenized_text', self.__start, self.__end))))
    ...
    def get_tweet_statistics(self):
        ...
        print('average tweet lenght: {0:.4}'.format(
            len(tokens) / len(self.data_full)))
        # max tweet len
        print('max tweet len: {}'.format(
            max([len(list_) for list_ in tokenized_text])))
    ...
    def print_line_graph(self, text_dict, imagename, size=50):
        ...
        fig.update_layout(
            title='{} ({} - {})'.format(imagename,
                                        self.__start.strftime('%b %Y'), self.__end.strftime('%b %Y')),
            xaxis_title='Words',
            yaxis_title='Tweets',
            yaxis_tickformat=',.1%',
        ...
        print(
            '> printed {}'.format(imagename))

    def print_trends_graph(self, imagename, entity_list=[], size=5):
        if entity_list == []:
            entity_list = [[item] for item in list(
                (self.get_token_dict()).keys())[0:size]]
        ...
        for month in range(n_months):
            ...
            tweets = self.field_compatter(
                'tokenized_text', date_start, date_end)
            for ent in entity_list:
                ...
                for tweet in tweets:
                    ...
                    if len(set(ent) & tweet) == len(set(ent)):
                        entity_stats['/'.join(ent)
                                     ][date_start.strftime('%m %Y')] += 1

        for ent in entity_list:
            val = [n/len(self.data_full)
...
def analyze(filename):
    ...
    analyzer.print_line_graph(analyzer.get_spacy_labels_stats(
        'LOC'), 'IMG/figure 2: loc entity distribution')
    analyzer.print_line_graph(analyzer.get_spacy_labels_stats(
        'ORG'), 'IMG/figure 3: org entity distribution')
    analyzer.print_line_graph(analyzer.get_spacy_labels_stats(
        'PER'), 'IMG/figure 4: per entity distribution')
    analyzer.print_line_graph(analyzer.get_spacy_labels_stats(
        'MISC'), 'IMG/figure 5: misc entity distribution')

    print('----')
    analyzer.print_trends_graph('IMG/figure 6: trends top 5 tokens')
    analyzer.print_trends_graph('IMG/figure 7: trends custom tokens', [
                                ['ue'], ['brexit'], ['inghilterra'], ['brexit', 'ue'], ['ue', 'gran bretagna'], ['ue', 'inghilterra'], ['inghilterra', 'euro']])
    print('----')
    analyzer.print_line_graph(analyzer.get_context_annotations_labels(),
                              'IMG/figure 8: context labels distribution')

    analyzer.print_line_graph(analyzer.get_context_label_stats(
        'brand'), 'IMG/figure 9: brands distribution')
    analyzer.print_line_graph(analyzer.get_context_label_stats(
        'person'), 'IMG/figure 10: person distribution')
    analyzer.print_line_graph(analyzer.get_context_label_stats(
        'entertainment'), 'IMG/figure 11: entertainment distribution')
    analyzer.print_line_graph(analyzer.get_context_label_stats(
        'sport'), 'IMG/figure 12: sport distribution')
    analyzer.print_line_graph(analyzer.get_context_label_stats(
        'politics'), 'IMG/figure 13: politics distribution')
    analyzer.print_line_graph(analyzer.get_context_label_stats(
    ...
    analyzer.print_line_graph(analyzer.get_entities_label_stats(
        'Place'), 'IMG/figure 18: Place entity annotations distribution')
    analyzer.print_line_graph(analyzer.get_entities_label_stats(
        'Organization'), 'IMG/figure 19: Organization entity annotations distribution')
    analyzer.print_line_graph(analyzer.get_entities_label_stats(
        'Product'), 'IMG/figure 20: Product entity annotations distribution')
    analyzer.print_line_graph(analyzer.get_entities_label_stats(
